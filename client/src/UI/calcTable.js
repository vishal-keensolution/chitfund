import React, {
  ...
  export default function calcTable(props) {
    ...
    function NPV(rate, initialCost, cashFlows=12) {
      ...
      print = true;
    if (print === true){
      var total_fund = props.monthly_chit_contribution;
      var moneypool_operating_cost = props.moneypool_operating_costs;
      ...
      for (let i = 0; i < moneypool_operating_cost_multiplier.length; i++) {
        ...
        if(security_deposit_cashback[i]>0){sdc = security_deposit_cashback[i]/100;}
         ...
         moc_inr[i]=moc_per[i]/100* (props.monthly_chit_contribution);
         participant_discount_per[i]=multiplier[i]/100*(props.base_discount_rate);
         participant_discount_inr[i]=(participant_discount_per[i]/100*(props.monthly_chit_contribution)).toFixed(0);
      ...
      for (var i = 0; i < 12; i++)
      {
        ...
        for (var j = 0; j < 12; j++) {
          if(i==j)
          {
            var p = parseFloat(total_fund)-parseFloat(moc_inr[i])-parseFloat(participant_discount_inr[i])-parseFloat(props.amount_per_chits);
            parti[i].push(p); 
          }
          else{
            var p=((parseFloat(props.amount_per_chits) - parseFloat(share_per_participant[i]))*(-1));
            parti[i].push(p);
          }  
        }
      }
      ...
      for (var i = 0; i < 12; i++)
      {
        ...
        for (var j = 0; j < 12; j++) {
          if(i==j)
          {
            var a = (parseFloat(parti[i][j]))-(parseFloat(props.value_of_scheme)-parseFloat(props.amount_per_chits));
            absolute_p[i].push(a); 
          }
          else{
      ...
      for (var i = 0; i < 12; i++)
      {
        ...
        for (var j = 0; j < 12; j++) {
          ...
          if(j>=i){
            proj[i].push((parseFloat(k)*(1/12)*(parseFloat(props.reinvestment_rate)/100)).toFixed(0)); 
          }else{proj[i].push("0");}
        }
        k=0;
      }
  ...
  //reinv_income_5      
      ...
      for (var i = 0; i < 12; i++)
      {
        for (var j = 0; j < 12; j++) {
          if(i==j)
          {
            if(buyback[i]==="No")
            {
              interest_cost_per2[i]=(parseFloat(income_participant[i])*100/(parseFloat(parti[i][j])+parseFloat(props.amount_per_chits))).toFixed(2);
            }
            else{
      ...
      for (var i = 0; i < 12; i++)
      {
        ...
        mirr[i]=MIRR(parseFloat(tot_of_parti[i]),parseFloat(moc_per[0]),parseFloat(props.reinvestment_rate));
    }
      console.log('moneypool_operating_cost',moneypool_operating_cost);
      console.log('monthly_chit_contribution',props.monthly_chit_contribution);
